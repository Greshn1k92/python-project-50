...
def format_value(node, depth: int) -> str:
    ...
    for key, value in sorted(node.items()):  # Sort for consistent order
        result.append(
            f'{big_indent}{key}: {format_value(value, depth + 1)}'
        )
...
def make_stylish_diff(diff_dict: dict, depth=0):
    """Generate a stylish representation of the diff, based on the user's
    working example."""
    big_indent = REPLACER * SPACE_COUNT * (depth + 1)
    ...
    for key in sorted(diff_dict.keys()):
        ...
        if action == "unchanged":
            result.append(
                f'{big_indent}{key}: {format_value(item["value"], depth + 1)}'
            )
        elif action == "changed":
            result.append(
                f'{sign_indent}- {key}: '
            ...
            result.append(
                f'{big_indent}{key}: '
                f'{make_stylish_diff(item["children"], depth + 1)}'
            )
...
# Keep original (potentially unused by stylish tests) functions format_diff and
# format. These were part of the original file structure but their logic might
# differ.
    ...
    for item_dict in diff_list: 
        ...
        if status == 'nested':
            old_school_result.append(
                f"{old_school_indent}{REPLACER * SPACE_COUNT}{key}: {{"
            )
            old_school_result.append(
                format_diff(item_dict.get('children', []), depth + 1)
            )
            old_school_result.append(
                f"{old_school_indent}{REPLACER * SPACE_COUNT}}}"
            )
        elif status == 'added':
            old_school_result.append(
                f"{old_school_indent}{REPLACER * (SPACE_COUNT - 2)}+ {key}: "
                f"{format_value(item_dict['value'], depth + 1)}"
            )
        elif status == 'removed':
            old_school_result.append(
                f"{old_school_indent}{REPLACER * (SPACE_COUNT - 2)}- {key}: "
            ...
            old_school_result.append(
                f"{old_school_indent}{REPLACER * (SPACE_COUNT - 2)}- {key}: "
                f"{format_value(item_dict['old_value'], depth + 1)}"
            )
            old_school_result.append(
                f"{old_school_indent}{REPLACER * (SPACE_COUNT - 2)}+ {key}: "
                f"{format_value(item_dict['new_value'], depth + 1)}"
            )
